...
class DFlashWorker:
    ...
    def __init__(
        ...
        # Build TpModelWorker kwargs, skipping unsupported params in older sglang
        _tp_kwargs = dict(
        ...
        # Add newer params only if TpModelWorker accepts them
        import inspect
        _tp_sig = inspect.signature(TpModelWorker.__init__)
        if 'attn_cp_rank' in _tp_sig.parameters:
            _tp_kwargs['attn_cp_rank'] = attn_cp_rank
        if 'moe_dp_rank' in _tp_sig.parameters:
            _tp_kwargs['moe_dp_rank'] = moe_dp_rank
