...
class DFlashWorkerV2(DFlashWorker):
    ...
    def __init__(
        ...
        # Scheduler constructs workers with scheduler-side kwargs (no attn_cp_rank /
        # moe_dp_rank); keep the V1 default-valued signature to stay compatible while
        # still allowing upstream callers that do provide the extra ranks.
        super().__init__(
    ...
    def forward_batch_generation(
        ...
        **kwargs,
        ...
        # DFlashDraftModel.forward returns the raw hidden tensor directly on this
        # branch (no LogitsProcessorOutput wrapper when capture_hidden_mode=NULL).
        if isinstance(draft_logits_output, torch.Tensor):
            draft_hidden = draft_logits_output
        else:
            draft_hidden = getattr(draft_logits_output, "hidden_states", None)
