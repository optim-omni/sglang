...
class DFlashDraftModel(nn.Module):
    ...
    def project_target_hidden(self, target_hidden: torch.Tensor) -> torch.Tensor:
        ...
        # Ensure output dtype matches model weights (avoid bf16/fp16 mismatch)
        if out.dtype != target_hidden.dtype:
            out = out.to(target_hidden.dtype)
        return out
    ...
    def forward(
        ...
        if input_embeds is None:
            # Warmup / dummy forward: create zero embeddings matching hidden_size
        ...
        # Ensure hidden_states dtype matches model params
